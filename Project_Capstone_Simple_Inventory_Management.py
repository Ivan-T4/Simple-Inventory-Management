    ...
    for id, details in inventory.items():
        ...
        if searchdata == 1:
            if key.upper() == id.upper():
                print_product(id, details) ####belum di check
                #print(f"{id:<15} | {details.get('Nama', 'N/A'):<15} | {details.get('Client', 'N/A'):<15} | {details.get('Tanggal', 'N/A'):<15} | {details.get('Grade', 'N/A'):<15} | {details.get('Stock', 'N/A'):<15}")
                found = True
...
masukkan angka untuk melanjutkan: """))
        if menuadd == 1:
            ...
            Tanggal = (input("Masukkkan Tanggal masuk Produk ke Gudang (cth: 25032025): ")) #Tanggal Masuk this should be like 250225 not 25022025
            Lebar = int(input("Masukkan lebar Produk(cm):"))
            Panjang = int(input("Masukkan lebar Produk(cm):"))
            Tinggi = int(input("Masukkan lebar Produk(cm):"))
...
Masukkan pilihan:"""))
                ...
                if processedit == 4:
                    ...
                    panjangs = int(input("Masukkan lebar Produk(cm):"))
                    tinggis = int(input("Masukkan lebar Produk(cm):"))
                    newvolume = newlebar*newpanjang*newtinggi
                    if newvolume >= 60000:
                    ...
                    newNama = oldID["Nama"] #keeping the old name
                    newClient = oldID["Client"] #keeping the old client 
                    newTanggal = oldID["Tanggal"] #keeping the old date
                    newGrade = Grades #keeping the new grade
                    newStock = oldID["Stock"] #keeping the old stock
                    newtinggi = tinggis #keeping the new height
                    newpanjang = panjangs #keeping the new length
                    newlebar = lebars #keeping the new width

                    # #creating details of newID
                    # inventory[newID]= {"Nama": oldID["Nama"], "Client": oldID["Client"], "Tanggal": oldID["Tanggal"], "Grade": newGrade, "Stock": oldID["Stock"], "Dimensi": {"Lebar": newlebar, "Panjang": newpanjang, "Tinggi": newtinggi}}
                    ...
                    if field2 == "Nama":
                        ...
                        newClient = oldID["Client"] #keeping the old client 
                        newTanggal = oldID["Tanggal"] #keeping the old date
                        newGrade = oldID["Grade"] #keeping the old grade
                        newStock = oldID["Stock"] #keeping the old stock
                        newtinggi = oldID["Dimensi"]["Tinggi"] #keeping the old height
                        newpanjang = oldID["Dimensi"]["Panjang"] #keeping the old length
                        newlebar = oldID["Dimensi"]["Lebar"] #keeping the old width
                    
                         #inventory[newID]= {"Nama": keyedit, "Client": oldID["Client"], "Tanggal": oldID["Tanggal"], "Grade": oldID["Grade"], "Stock": oldID["Stock"], "Dimensi": {"Lebar": oldID["Dimensi"]["Lebar"], "Panjang": oldID["Dimensi"]["Panjang"], "Tinggi": oldID["Dimensi"]["Tinggi"]}}
                    elif field2 == "Client":
                        #oldID = inventory[editbyID]
                        ...
                        newpanjang = oldID["Dimensi"]["Panjang"] #keeping the old length
                        newlebar = oldID["Dimensi"]["Lebar"] #keeping the old width
                         #inventory[newID]= {"Nama": oldID["Nama"], "Client": keyedit, "Tanggal": oldID["Tanggal"], "Grade": oldID["Grade"], "Stock": oldID["Stock"], "Dimensi": {"Lebar": oldID["Dimensi"]["Lebar"], "Panjang": oldID["Dimensi"]["Panjang"], "Tinggi": oldID["Dimensi"]["Tinggi"]}}
                    else:
                        #oldID = inventory[editbyID]
                        ...
                        newlebar = oldID["Dimensi"]["Lebar"] #keeping the old width
                         ...
                         #inventory[newID]= {"Nama": oldID["Nama"], "Client": oldID["Client"], "Tanggal": keyedit, "Grade": oldID["Grade"], "Stock": oldID["Stock"], "Dimensi": {"Lebar": oldID["Dimensi"]["Lebar"], "Panjang": oldID["Dimensi"]["Panjang"], "Tinggi": oldID["Dimensi"]["Tinggi"]}}
